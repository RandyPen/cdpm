...
- **Protocol / agent** callers cap `scoinAmount` at `min(neededWrapper, entry.wrapperRaw − LENDING_SAFE_MARGIN_WRAPPER_RAW)` (recommended default `100n` sCoin raw). Never pass `u64::MAX`. Use a pure helper — see the `capRedeemBurnRaw` example in [`kai-lending-math.md` §9.1](./kai-lending-math.md#91-full-drain-dust-and-the-lending_safe_margin_wrapper_raw-floor); the helper is protocol-agnostic. Scallop's exact floor-div means no `coin::join` topup is *required* here (unlike Kai, where it is mandatory on **every** redeem); cdpm's worker nonetheless merges the same minimal **~3 raw** topup from its own wallet on the Scallop path for parity (one code path for both protocols) — keep it minimal since it is donated into `pm.balance[T]` per redeem.
