...
>
> - **Scallop entries** — keyed by `type_name<T>`, value `ScallopVault<T> { scoin: Balance<MarketCoin<T>>, principal: u64 }`. The sCoin type is structurally pinned to `MarketCoin<T>` by the type system. To value, you need a Scallop reserve snapshot — see [`scallop-lending-math.md`](./scallop-lending-math.md) for `predictScallopRedeem` and the principal / yield-fee split.
> - **Kai SAV entries** — keyed by `type_name<YT>`, value `KaiVault<T, YT> { yt_balance: Balance<YT>, principal: u64 }`. To value, you need a Kai vault snapshot — see [`kai-lending-math.md`](./kai-lending-math.md) for `predictKaiWithdraw` and the (identical) principal / yield-fee split.
>
