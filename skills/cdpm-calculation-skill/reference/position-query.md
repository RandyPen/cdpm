...
> The `PositionManager` Move struct has a fourth bag, `lending: Bag`, keyed by underlying `type_name<T>` and storing per-(T, S) `ScallopVault { scoin: Balance<S>, principal: u64 }`. To value the lending portion you also need a Scallop reserve snapshot — see `reference/scallop-lending-math.md` for the off-chain twins of `compute_expected_underlying` and the principal/yield-fee split.
