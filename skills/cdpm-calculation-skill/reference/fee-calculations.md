...
cdpm uses a single `FeeHouse.fee_rate` (basis points, capped at `MAX_FEE_RATE = 5000` = 50%, default `2000` = 20%) for **three** distinct paths:
...
2. **Scallop yield fee** — `scallop_redeem` deducts only from the interest portion:
...
3. **Kai SAV yield fee** — `kai_redeem` uses the **same** interest-only formula as Scallop (only the source of `redeemedAmount` and `principalPortion` differs — Kai's `vault::withdraw → klsp::withdraw → vault::redeem_withdraw_ticket` chain rather than Scallop's `redeem::redeem`). There is no separate Kai fee rate.

All three formulas floor and share the same `feeRateBp` from `FeeHouse`. See [`scallop-lending-math.md`](./scallop-lending-math.md) and [`kai-lending-math.md`](./kai-lending-math.md) for the full redeem predictions (principal amortization, `predictScallopRedeem` / `predictKaiWithdraw`, end-to-end helpers).
