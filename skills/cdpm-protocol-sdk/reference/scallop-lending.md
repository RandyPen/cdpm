...
`scoinToBurnForTargetNet` may return `MAX_U64` when the `ScallopVault<T>` entry cannot satisfy `desiredNet`. Scallop's upstream `protocol::redeem::redeem` shares its single floor-div formula with cdpm's `compute_expected_underlying_scallop`, so `redeemed_amount == expected_underlying` exactly in the common case — and cdpm's `scallop_finish_redeem` additionally tolerates up to `REDEEM_DUST_TOLERANCE_RAW = 4` raw on-chain as forward-compatibility. **Protocol callers should still cap the burn** to leave a residual entry behind (the bag-key reason), not for the dust concern. See [`cdpm-calculation-skill/reference/scallop-lending-math.md` §9.1](../../cdpm-calculation-skill/reference/scallop-lending-math.md#91-floor-div-dust-on-chain-tolerance-and-the-lending_safe_margin_wrapper_raw-floor) for the math.
...
**Dust topup not needed.** Scallop redeems return exactly `expected_underlying` today (shared floor-div); the on-chain `REDEEM_DUST_TOLERANCE_RAW` further absorbs any future rounding drift up to 4 raw. No `coin::join` topup is required on the Scallop path, and the Kai path is equally topup-free now (see [`kai-lending.md` "Floor-div dust"](./kai-lending.md)).
