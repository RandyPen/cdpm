...
async function handleAgentError(error: any): Promise<string> {
  ...
  if (errorStr.includes('ENotOwner')) {
    ...
    return 'finish_* received Coin with value < ticket.expected. Kai: EVERY redeem (partial OR full) needs a coin::join dust topup — the strategy walk surfaces a few raw units below ticket.expected, and over-sizing the burn cannot escape it (expected and surfaced scale together). Merge a small Coin<T> (≈ 3 raw, the single-strategy dust bound — keep it minimal, the whole topup is donated into pm.balance every redeem) from the AGENT WALLET before kai_finish_redeem; cdpm has no agent-tier call that yields a Coin<T> from PM bags, so the agent wallet must hold a little of each redeemable underlying (SUI splits from tx.gas). Also cap the burn at wrapperRaw − LENDING_SAFE_MARGIN_WRAPPER_RAW. Scallop: redeemed == expected exactly (shared floor-div), so usually a missing accrue_interest_for_market as PTB command 0 or stale reserve state — re-snapshot after accrue; the same topup is defensive-only there. See cdpm-calculation-skill/reference/{kai,scallop}-lending-math.md §9.1 and cdpm-agent-sdk/reference/{kai,scallop}-lending.md "Floor-div dust".';
