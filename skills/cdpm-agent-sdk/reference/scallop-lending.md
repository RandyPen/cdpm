...
**Dust topup (defensive).** Unlike Kai — where the strategy walk forces a **mandatory** `coin::join` topup on *every* redeem (see [`kai-lending.md` "Floor-div dust"](./kai-lending.md)) — Scallop's `redeem::redeem` shares cdpm's exact floor-div formula, so `redeemed == expected` and no topup is normally required. For parity, cdpm's own worker merges the **same tiny `Coin<T>` topup (≈ 3 raw, from the agent wallet)** before `scallop_finish_redeem` on the Scallop path too. Keep it minimal — the whole topup is donated into `pm.balance[T]`, so an oversized topup on a frequently-reconciling agent accumulates. Agents may omit it for Scallop, but wiring it identically for both protocols is the simpler, safer default.
