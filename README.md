...
> **Note:** This branch (`feat/scallop-lending`) introduces the Scallop lending integration, which adds a new `lending: Bag` field to `PositionManager` and several new public entrypoints (`scallop_start_supply` / `scallop_finish_supply` / `scallop_start_redeem` / `scallop_finish_redeem` / `user_extract_scallop_market_coin`). This is a **breaking layout change** — the previously published immutable package can no longer be reused, and the addresses above must be regenerated by a fresh deploy.
